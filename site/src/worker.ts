...
function return_html_index(headlines_ul: string, sources: string): string {
    return `<html>
<head>
...
async function get_sources(env: Env, auth_headers: Object, hours: number = 24): Promise<Array> {
    ...
    for (const row of rows) {
        sources.push({'source_name': row[0], 'volume': row[1]});
    }
    return sources;
}
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        if (url.searchParams.get('source_type') !== null) {
          const source_name = url.searchParams.get('source_type');
